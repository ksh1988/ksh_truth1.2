...
最终生成的代码里不允许出现 Unicode 转义文案，例如反斜杠加字母 u 的编码形式。
...
不允许因为编码问题生成乱码文案，例如连续问号占位符、UTF-8 被错误解码后的中文乱码、年份后面多出英文问号这类内容。发现乱码必须立刻修复。
...
新增或修改组件时，组件脚本顶部必须写清楚组件职责。

## 编译前检查规则

每次运行 `npm run build` 前，必须先运行文本完整性检查：

```bash
npm run check:text
```

现在 `package.json` 已经把 `build` 改为：

```bash
npm run check:text && vite build
```

所以以后正常执行 `npm run build` 会自动先检查文本。

文本检查脚本位置：

```text
scripts/check-text-integrity.js
```

检查范围只包括源码、站点数据和文档规则，不扫描：
- `dist/`
- `node_modules/`
- `.git/`
- `src/images/`
...
- 反斜杠加字母 u 的 Unicode 转义文案
- 年份后拼接英文问号的代码
- 常见 UTF-8 被错误解码后的中文乱码片段
...
本项目包含大量中文、韩文和日文。以后读取或检查文本文件时，默认不要只用普通 PowerShell `Get-Content` 判断文本是否乱码，因为它可能把 UTF-8 显示成假乱码。
...
检查乱码时，优先使用 `npm run check:text` 或 Node/Python 按 UTF-8 验证，不要只凭 PowerShell 控制台显示判断。
